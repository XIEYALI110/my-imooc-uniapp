		...
		<view class="home-list">
			<view v-if="is_histroy" class="label-box">
				...
				<view v-if="historyList.length > 0" class="label-content">
					<view class="label-content__item" v-for="(item,index) in  historyList"  :key="index" @click="openHistroy(item)">{{item.name}}</view>
				</view>
			...
			<list-scroll v-else class="list-scroll">
				...
				<view v-if="searchList.length > 0">
					<list-card :item="item" v-for="item in searchList" :key="item._id" @click="setHistory" @clickDes="clickDes"></list-card>
				</view>
